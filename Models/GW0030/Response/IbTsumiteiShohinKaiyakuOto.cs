namespace WebAPIJsonDataMaker.Models.GW0030.Response
{
    public class IbTsumiteiShohinKaiyakuOto
    {
        public string shoribi { get; set; }
        public string shoriJikoku { get; set; }
        public string yonyubi { get; set; }
        public int shohinShubetsu { get; set; }
        public string shohinMeisho { get; set; }
        public int nyukinKozaTemban { get; set; }
        public string nyukinKozaTenmei { get; set; }
        public string nyukinKozaTenmeiKana { get; set; }
        public int nyukinKozaKamokuCode { get; set; }
        public string nyukinKozaKamokumei { get; set; }
        public int nyukinKozabango { get; set; }
        public int teikiTemban { get; set; }
        public string teikiTenmei { get; set; }
        public string teikiTenmeiKana { get; set; }
        public int teikiKamokuCode { get; set; }
        public int teikiKamokuCodechiwake { get; set; }
        public string teikiKamokumei { get; set; }
        public int teikiKozabango { get; set; }
        public int yonyuKikan { get; set; }
        public string mankibi { get; set; }
        public decimal? riritsu { get; set; }
        public int keizokuKubun { get; set; }
        public string keizokuKubunMeisho { get; set; }
        public int kazeiKubun { get; set; }
        public string kazeiKubunMeisho { get; set; }
        public long uketsukeKingaku { get; set; }
        public long risoku { get; set; }
        public long sashihikiShiharaiKingaku { get; set; }
        public long zeiGokei { get; set; }
        public long uchiKokuzei { get; set; }
        public long uchiChihozei { get; set; }
        public long sogoKozaFurikaeKingaku { get; set; }
        public long kashikoshiRisoku { get; set; }
        public long teikiyokinHurikaeKingaku { get; set; }
        public long shiteiKozaFurikaeKingaku { get; set; }
        public string shiharaibi { get; set; }
        public long gankin { get; set; }
        public int yonyuBango { get; set; }
        public int kakuninJikkoKubun { get; set; }
        public int keiyakushaId { get; set; }
        public int meisaiBango { get; set; }
    }
}