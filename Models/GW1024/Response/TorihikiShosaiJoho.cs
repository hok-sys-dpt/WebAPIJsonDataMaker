namespace WebAPIJsonDataMaker.Models.GW1024.Response
{
    public class TorihikiShosaiJoho
    {
        public long kingaku { get; set; }
        public int teikiKanrenKozaTemban { get; set; }
        public string teikiKanrenKozaTenmei { get; set; }
        public int teikiKanrenKozaKamokuCode { get; set; }
        public int teikikanrenKozaBango { get; set; }
        public int teikiTemban { get; set; }
        public string teikiTenmei { get; set; }
        public int teikiKamokuCode { get; set; }
        public int teikiKamokuCodechiwake { get; set; }
        public int teikiKozabango { get; set; }
        public int shohinShubetsu { get; set; }
        public int shohinUchiwake2 { get; set; }
        public int shohinUchiwake3 { get; set; }
        public int yonyuKikan { get; set; }
        public string yonyuKikanMongon { get; set; }
        public int keizokuKubun { get; set; }
        public int kazeiKubun { get; set; }
        public string kazeiKubunMeisho { get; set; }
        public string yonyubi { get; set; }
        public string mankibi { get; set; }
        public int mankiAnnaiYohuyoKubun { get; set; }
        public decimal? riritsu { get; set; }
        public int yonyuBango { get; set; }
        public long yonyuZandaka { get; set; }
        public int risokuKeisanshoHyoji { get; set; }
    }
}